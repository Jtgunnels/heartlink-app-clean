// ---------------------------------------------------------------------------
// HeartLink Algorithm v3.8 — Optimized Configuration Lock (2025-10-12)
// ---------------------------------------------------------------------------
// Includes: noise guard, adaptive EMA extension, and yellow stickiness
// ---------------------------------------------------------------------------

export const CONFIG_V38 = Object.freeze({
  EMA_WINDOW_DAYS: 5,
  DEESCALATION_DAYS: 2,
  MILD_WEIGHT_FACTOR: 0.9,
  VERSION: "3.8.OPT2025-10-12",
  NOISE_THRESHOLD: 0.3,          // max tolerated random fluctuation per symptom
  EXTENDED_EMA_WINDOW: 9,        // used for high jitter cases
  YELLOW_STICKINESS_DAYS: 2      // days to hold before returning Green
});
...
export default function calculateScore_v3_8(input = {}, baseline = {}, hist = {}, debug = false) {
  ...
  const highNoise = diffs.every(d => d <= CONFIG_V38.NOISE_THRESHOLD) && avgNoise > 0;

  // If all differences are small and stable → noise guard active
  let noiseGuard = false;
  if (highNoise && (hist.categories ?? []).every(c => c === "Green" || c === "Yellow")) {
    noiseGuard = true;
  }
  ...
  const weightedSymptoms =
    (base.sob + base.edema + base.fatigue + base.orthopnea + base.palpitations) *
  ...
  const EMA_DAYS =
    jitterVariance > 0.8 ? CONFIG_V38.EXTENDED_EMA_WINDOW : CONFIG_V38.EMA_WINDOW_DAYS;
