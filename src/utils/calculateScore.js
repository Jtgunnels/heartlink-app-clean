/**
 * -------------------------------------------------------------------------
 * HeartLink Health LLC
 * Algorithm: calculateScore_v41CL.js
 * Version: ASE 1.3 (Clinical-Lock)
 * Device Type: Wellness / Remote Monitoring Support (Non-diagnostic)
 * Software Class: SaMD, Class I (exempt)
 * Validation: Trials T1.0–T1.9 (see DHF HL-V4.1-DHF-001)
 * Verification: 100 % automated unit coverage, deterministic output
 * Change Control: Locked – no further parameter tuning permitted
 * -------------------------------------------------------------------------
 */
...
export default function calculateScore_v41CL(entry, baseline, hist = {}, cfg = CFG_V41CL) {
  ...
  if (cfg.ENABLE_L3) {
    ...
    // ✅ Only apply if both are finite valid numbers
    if (Number.isFinite(wt) && Number.isFinite(wb)) {
      const d = wt - wb;
    ...
    // ✅ Short 3-day change only if last 3 are valid finite numbers
    const wHist = (hist.weights ?? []).filter(Number.isFinite).slice(-3);
    if (wHist.length === 3) {
  ...
  hist.weights = [
  ...(hist.weights ?? []),
  Number.isFinite(entry.weightToday)
    ? entry.weightToday
    : (Number.isFinite(baseline.baselineWeight) ? baseline.baselineWeight : null),
];
